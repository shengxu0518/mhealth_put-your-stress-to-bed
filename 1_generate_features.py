...
for subj in SETTINGS['SUBJECT_LIST']:
    ...
    segmentation_in_index = pd.DataFrame({'Start': timeToIndex(segmentation_in_time['Start'], data['Time'].as_matrix()),
                                          'End':   timeToIndex(segmentation_in_time['End'], data['Time'].as_matrix())},
                                         columns=['Start', 'End'])
